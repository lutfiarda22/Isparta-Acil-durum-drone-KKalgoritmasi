...
# ----------------------------------------
# 🎨 CSS – Sidebar (Parametreler) Lacivert
# ----------------------------------------
st.markdown(
    """
    <style>
    ...
    section[data-testid="stSidebar"] {
        background-color: #0a1a2f;  /* Lacivert */
    }
...
# Başlık & Açıklama
# ----------------------------------------
st.markdown("## Isparta Acil Durum Drone Rota Optimizasyonu")
st.markdown(
    "Bu uygulamada **Google Maps API** üzerinden alınan gerçek mesafeler kullanılarak, "
    "**Karınca Kolonisi Optimizasyonu (ACO)** algoritması ile "
    "acil durumlar için en kısa drone rotası hesaplanmaktadır."
)
...
# ----------------------------------------
st.sidebar.header("KKO Parametreleri")
...
if st.session_state.result is not None:
    ...
    improvement_rate = ((initial_distance - best_distance) / initial_distance) * 100
    ...
    for i in range(len(best_route)):
        ...
        table_data.append({
            ...
            "Mesafe (km)": round(distance_matrix[current_idx][next_idx], 4)
        })
    ...
    st.info("Parametreleri ayarlayıp **Optimizasyonu Başlat** düğmesine tıklayın.")
